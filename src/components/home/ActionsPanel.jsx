...
const ActionsPanel = () => {
  ...
  const filteredActions = user.role === "Developer"
    ? actions.filter((action) => !["Analytics", "Settings"].includes(action.name))
    : actions;
    ...
    <>
      ...
      <section aria-labelledby="quick-links-title">
        <div className="divide-y divide-gray-200 overflow-hidden rounded-lg bg-gray-200 shadow sm:grid sm:grid-cols-2 sm:gap-px sm:divide-y-0">
          ...
          {LinksActions.map((action, actionIdx) => (
            repo &&
            files && (
              <div
                key={action.name}
                className={classNames(
                  ...
                  actionIdx === LinksActions.length - 1 ? " sm:rounded-bl-none" : "",
                  "relative group bg-white p-6 focus-within:ring-2 focus-within:ring-inset focus-within:ring-cyan-500"
                )}
              >
                <a href={actionIdx === 0 ? repo : files}>
                  <span
                    className={classNames(
          ...
          {filteredActions.map((action, actionIdx) => (
            <div
              ...
              className={classNames(
                ...
                actionIdx === filteredActions.length - 1 ? "rounded-bl-lg rounded-br-lg sm:rounded-bl-none" : "",
                "relative group bg-white p-6 focus-within:ring-2 focus-within:ring-inset focus-within:ring-cyan-500"
              )}
            >
              <div>
